<template>
    <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">التصنيفات</h1>
            <Button v-if="can('categories.create')" label="إضافة تصنيف" icon="pi pi-plus" @click="openCreateDialog" />
        </div>


        <!-- Data Table -->
        <CategoriesTable :items="store.items" :loading="store.loading" @edit="openEditDialog" @delete="confirmDelete" />
        ...
        <!-- Create/Edit Dialog -->
        <Dialog v-model:visible="dialogVisible" :header="isEditing ? 'تعديل التصنيف' : 'إضافة تصنيف'"
            ...
            <template #footer>
                ...
                <Button :label="isEditing ? 'تحديث' : 'حفظ'" :loading="saving" @click="saveCategory" />
            </template>
        </Dialog>
        ...
        <Dialog v-model:visible="deleteDialogVisible" header="تأكيد الحذف" :style="{ width: '400px' }" modal>
            <div class="flex items-center gap-4">
                ...
                <span>هل أنت متأكد من حذف التصنيف "{{ itemToDelete?.title }}"؟</span>
            </div>
            <template #footer>
                <Button label="إلغاء" severity="secondary" text @click="deleteDialogVisible = false" />
                <Button label="حذف" severity="danger" :loading="deleting" @click="deleteCategory" />
            </template>
        </Dialog>
...
import CategoriesTable from '../../components/letters/CategoriesTable.vue'
    ...
    action
        .then(() => {
            toast.add({
                severity: 'success',
                summary: 'نجاح',
                detail: isEditing.value ? 'تم تحديث التصنيف بنجاح' : 'تم إنشاء التصنيف بنجاح',
                life: 3000
            })
            closeDialog()
    ...
    store.remove(itemToDelete.value.id)
        ...
        .catch(() => {
            toast.add({
                severity: 'error',
                summary: 'خطأ',
                detail: store.error || 'فشل في حذف التصنيف',
                life: 3000
            })
        })
        .finally(() => {
