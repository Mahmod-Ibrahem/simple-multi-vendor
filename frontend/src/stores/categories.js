...
export const useCategoryStore = defineStore('categories', () => {
    ...
    function fetchAll(page = 1) {
        ...
        return api.get('/categories', { params: { page, per_page: pagination.value.perPage } })
            .then((response) => {
            ...
            .catch((err) => {
                error.value = err.response?.data?.message || 'فشل في جلب التصنيفات'
                throw err
            })
            .finally(() => {
                loading.value = false
    ...
    function fetchAllForDropdown() {
        return api.get('/categories', { params: { paginate: false } })
            .then((response) => {
    ...
    function fetchOne(id) {
        ...
        return api.get(`/categories/${id}`)
            ...
            .catch((err) => {
                error.value = err.response?.data?.message || 'فشل في جلب التصنيف'
                throw err
            })
            .finally(() => {
                loading.value = false
