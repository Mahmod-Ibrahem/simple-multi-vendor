<template>
    <div class="card overflow-hidden">
                    ...
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 flex items-center justify-center font-bold text-sm border border-purple-200 dark:border-purple-800/50">
                                    {{ item.title?.charAt(0) }}
                                </div>
                                <span class="font-semibold text-slate-700 dark:text-slate-200">{{ item.title }}</span>
                            </div>
                        </td>
                        <td class="text-slate-500 dark:text-slate-400">{{ item.description || '-' }}</td>
                        <td>
                            <div class="flex items-center gap-2">
                                <button v-if="can('assignments.update')" @click="$emit('edit', item)"
                                ...
                                <button v-if="can('assignments.delete')" @click="$emit('delete', item)"
        ...
        <div v-else class="p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">لا توجد جهات مكلفة</h3>
            <p class="text-slate-600 dark:text-slate-400">ابدأ بإضافة جهة مكلفة جديدة</p>
        </div>
    </div>
</template>
