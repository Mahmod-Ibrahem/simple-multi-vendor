<template>
    <div class="card overflow-hidden">
                    ...
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 flex items-center justify-center font-bold text-sm border border-rose-200 dark:border-rose-800/50">
                                    {{ item.title?.charAt(0) }}
                                </div>
                        ...
                        <td>
                            <div class="flex items-center gap-2">
                                ...
                                <button @click="$emit('delete', item)" :disabled="item.is_default" :class="[
                                    'transition-colors',
                                    item.is_default
        ...
        <div v-else class="p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">لا توجد حالات</h3>
            <p class="text-slate-600 dark:text-slate-400">ابدأ بإضافة حالة جديدة</p>
        </div>
    </div>
</template>
