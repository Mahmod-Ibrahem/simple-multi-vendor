<template>
    <div class="card overflow-hidden">
                    ...
                    <tr v-for="item in items" :key="item.id">
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400 flex items-center justify-center font-bold text-sm border border-teal-200 dark:border-teal-800/50">
                                    {{ item.title?.charAt(0) }}
                                </div>
                        ...
                        <td>
                            <div class="flex items-center gap-2">
                                <button v-if="can('subjects.update')" @click="$emit('edit', item)"
                                ...
                                <button v-if="can('subjects.delete')" @click="$emit('delete', item)"
        ...
        <div v-else class="p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">لا توجد مواضيع</h3>
            <p class="text-slate-600 dark:text-slate-400">ابدأ بإضافة موضوع جديد</p>
        </div>
    </div>
</template>
